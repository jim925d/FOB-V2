import { Card, CardContent } from "@/components/ui/card";
import PageHero from "@/components/layout/PageHero";
import { useApi } from "@/hooks/useApi";
import { getCommunities } from "@/lib/api";

function toList(data) {
  if (!data) return [];
  if (Array.isArray(data)) return data;
  return data.items ?? data.communities ?? data.results ?? [];
}

export default function CommunitiesPage() {
  const { data: rawData, loading, error } = useApi(getCommunities);
  const list = toList(rawData);

  return (
    <>
      <PageHero
        eyebrow="Communities"
        title="Veteran Communities"
        subtitle="Organizations and groups for veterans and military families."
      />
      <div className="max-w-[900px] mx-auto px-7 py-8">
        {loading && (
          <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>Loading communities…</p>
        )}
        {error && (
          <Card className="mb-6" style={{ background: "var(--color-bg-secondary)", borderColor: "var(--color-border)" }}>
            <CardContent className="pt-5">
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>{error}</p>
              <p className="text-xs mt-2" style={{ color: "var(--color-text-dim)" }}>
                Set VITE_API_BASE in .env and ensure the API is running.
              </p>
            </CardContent>
          </Card>
        )}
        {!loading && !error && (
          <>
            {list.length === 0 ? (
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>
                No communities returned. Check API response shape (e.g. items, communities).
              </p>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2">
                {list.slice(0, 24).map((c, i) => (
                  <Card
                    key={c.id ?? c.slug ?? i}
                    style={{ background: "var(--color-bg-secondary)", borderColor: "var(--color-border)" }}
                  >
                    <CardContent className="pt-4 pb-4">
                      <div className="font-medium" style={{ color: "var(--color-text-primary)" }}>
                        {c.name ?? c.title ?? "Unnamed"}
                      </div>
                      {(c.description ?? c.summary ?? c.website) && (
                        <p className="text-sm mt-1 line-clamp-2" style={{ color: "var(--color-text-muted)" }}>
                          {c.description ?? c.summary ?? c.website}
                        </p>
                      )}
                    </CardContent>
                  </Card>
                ))}
              </div>
            )}
          </>
        )}
      </div>
    </>
  );
}
